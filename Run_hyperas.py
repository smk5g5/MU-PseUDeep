...
def data():
    ...
    file1 = open("./trainx1", 'rb') #sec str one hot matrix
    trainX1 = pickle.load(file1)
    file1 = open("./trainx2", 'rb') #seq one hot matrix
    trainX2 = pickle.load(file1)
    file1 = open("./trainy", 'rb') #labels
    ...
    file1 = open("./testx1", 'rb')
    testX1 = pickle.load(file1)
    file1 = open("./testx2", 'rb')
...
##    from tensorflow.keras.callbacks import TensorBoard
    ...
    checkpointer = ModelCheckpoint(filepath='%d-secstr_seq_denseconcat.h5' % i, verbose=1,save_best_only=True, monitor='val_loss', mode='min')
    fitHistory = cnn.fit([trainX1,trainX2], trainY1, batch_size={{choice([32,64,128,256,512])}}, nb_epoch=500,validation_data=([valX1,valX2], valY1),callbacks=[checkpointer,early_stopping,tensorboard])
    myjson_file = "myhist_" +"_dict" + "_fold_" +str(i)
