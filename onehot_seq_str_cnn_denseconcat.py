...
def run_test_onehot(test,iteration):
    ...
    pickle.dump(testX1, open( "./testx1", "wb" ))
    pickle.dump(testX2, open( "./testx2", "wb" ))
    pickle.dump(testY, open( "./testy", "wb" ))
    pre_score = test_in(testX1,testX2, testY, iteration, 'combined_model')
    print('test loss:', pre_score[0],'test acc:', pre_score[1])

i =0
#Nm2Ome_test_combined.txt  Nm2Ome_training_cdhit2_combined.txt
train = read_composite_data("./emboss_60perc_train_0.txt")
train = train.sample(frac=1)
test =  read_composite_data("./emboss_60perc_test_0.txt")
...
seq,secstr,label = get_all_seq(train2)
...
#trainX2.shape = (trainX2.shape[0], row2,col1)
...
#trainX1,trainX2,trainY1,valX1,valX2,valY1,input_1,input_2, i
...
pickle.dump(trainX1, open( "./trainx1", "wb" ))
pickle.dump(trainX2, open( "./trainx2", "wb" ))
pickle.dump(trainY, open( "./trainy", "wb" ))
pickle.dump(ValX1, open( "./valx1", "wb" ))
pickle.dump(ValX2, open( "./valx2", "wb" ))
pickle.dump(ValY, open( "./valy", "wb" ))

run_test_onehot(test_new,i)
